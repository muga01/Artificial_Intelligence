...
class Graph:
    def __init__(self, vertices):
        ...
        # self.visitedList = [] # keep track of all the dfs paths
    ...
    def depthFirst(self, s, visited=[]):
        visited.append(s)
        for vertex in self.edge(s):
            if vertex not in visited:
                self.depthFirst(vertex, visited.copy())

            if len(set(visited)) == self._vertices:
    ...
    def short_path_dfs(self, start):
        shortest = np.inf
        shortest_path = []
        # self.depthFirst(start)
        all_paths = self.depthFirst(start)
        # print(len(all_paths))
        for paths in all_paths:
            # print(len(set(paths)))
            paths = paths + [start]
            distance = 0
            for root in range(len(paths) - 1):
                if self._adjMatrix[paths[root]][paths[root + 1]] != 0:
                    distance += self._adjMatrix[paths[root]][paths[root + 1]]
                else:
                    shortest = np.inf
                    break
        ...
        if shortest != np.inf:
            # print("Path: {0} Cost: {1}".format(shortest_path, shortest))
